      ...
      <eo:RsData>
        <eo:Account>
          <com:AccountIdentifier>
            <com:AccountID>
              <xsl:value-of select="/app:FIXML/app:Body/app:executeFinacleScriptResponse/app:executeFinacleScript_CustomData/app:REFNUM"/>
            </com:AccountID>
          </com:AccountIdentifier>
        </eo:Account>
        <xsl:choose>
          <xsl:when test="count (/app:FIXML/app:Body/app:Error ) &lt;= 0">
            <com:Status>
              ...
              <com:Text>Virtual account processed successfully</com:Text>
            </com:Status>
          </xsl:when>
          <xsl:otherwise>
            <com:Status>
