...
<xsl:stylesheet version="1.0" xmlns:oraxsl="http://www.oracle.com/XSL/Transform/java"
  ...
  <xsl:template match="/">
    <eo:ESBMsg>
      ...
      <eo:RsData>
        <xsl:for-each select="/app:FIXML/app:Body/app:executeFinacleScriptResponse/app:executeFinacleScript_CustomData/app:billers">
          <xsl:if test="app:BG/app:billercode">
            <eo:Biller>
              <com:BillerIdentifier>
                <xsl:if test="app:BG/app:bankId">
                  <com:BankID>
                    <xsl:value-of select="app:BG/app:bankId"/>
                  </com:BankID>
                </xsl:if>
                <xsl:if test="app:BG/app:billAcctId">
                  <com:AccountID>
                    <xsl:value-of select="app:BG/app:billAcctId"/>
                  </com:AccountID>
                </xsl:if>
                <xsl:if test="app:BG/app:billerId">
                  <com:BillerID>
                    <xsl:value-of select="app:BG/app:billerId"/>
                  </com:BillerID>
                </xsl:if>
                <xsl:if test="app:BG/app:billercode">
                  <com:BillerCode>
                    <xsl:value-of select="app:BG/app:billercode"/>
                  </com:BillerCode>
                </xsl:if>
              </com:BillerIdentifier>
              <eo:BillerName>
                <xsl:value-of select="app:BG/app:billAcName"/>
              </eo:BillerName>
            </eo:Biller>
          </xsl:if>
        </xsl:for-each>
        <xsl:choose>
          <xsl:when test="count (/app:FIXML/app:Body/app:Error ) &lt;= 0">
            <com:Status>
              ...
              <com:Text>Biller code queried successfully</com:Text>
            </com:Status>
          </xsl:when>
          <xsl:otherwise>
            <com:Status>
