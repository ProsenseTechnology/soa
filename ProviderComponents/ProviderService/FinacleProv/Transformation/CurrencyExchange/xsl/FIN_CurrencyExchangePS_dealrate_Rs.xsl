...
<xsl:stylesheet version="1.0" xmlns:oraxsl="http://www.oracle.com/XSL/Transform/java"
  ...
  <xsl:template match="/">
    <eo:ESBMsg>
      ...
      <eo:RsData>
        <xsl:for-each select="/app:FIXML/app:Body/app:executeFinacleScriptResponse/app:executeFinacleScript_CustomData/app:ACCTLISTStruct/app:Acct">
          <eo:CurrencyExchange>
            ...
            <com:CurrencyExchangeIdentifier>
              <com:CurrencyExchangeID>
                <xsl:value-of select="app:DealRefNo"/>
              </com:CurrencyExchangeID>
            </com:CurrencyExchangeIdentifier>
            <eo:ConversionRate>
              <xsl:value-of select="app:DealRate"/>
            </eo:ConversionRate>
            <eo:ConversionDateTime>
              <xsl:value-of select="app:DealDate"/>
            </eo:ConversionDateTime>
            <eo:FromCurrencyCode>
              <xsl:value-of select="app:DealCurr"/>
            </eo:FromCurrencyCode>
            <eo:ToCurrencyCode>
              <xsl:value-of select="app:DealCurrTo"/>
            </eo:ToCurrencyCode>
            <com:Amount>
              <xsl:value-of select="app:DealAmt"/>
            </com:Amount>
            <com:BankID>
