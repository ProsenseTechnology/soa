...
<xsl:stylesheet version="1.0" xmlns:oraxsl="http://www.oracle.com/XSL/Transform/java"
  ...
  <xsl:template match="/">
    <eo:ESBMsg>
      ...
      <eo:RsData>
        <eo:CurrencyExchange>
          <com:Amount currencyCode="{/app:FIXML/app:Body/app:getExchangeRateForRateCodeResponse/app:ExchangeRateForRateCodeOutputVO/app:outputAmount/app:currencyCode}">
            <xsl:value-of select="/app:FIXML/app:Body/app:getExchangeRateForRateCodeResponse/app:ExchangeRateForRateCodeOutputVO/app:outputAmount/app:amountValue"/>
          </com:Amount>
          <eo:FixedCurrencyUnit currencyCode="{/app:FIXML/app:Body/app:getExchangeRateForRateCodeResponse/app:ExchangeRateForRateCodeOutputVO/app:fxdCrncyCode}">
            <xsl:value-of select="/app:FIXML/app:Body/app:getExchangeRateForRateCodeResponse/app:ExchangeRateForRateCodeOutputVO/app:fxdCrncyUnits"/>
          </eo:FixedCurrencyUnit>
          <eo:VarCurrencyUnit currencyCode="{/app:FIXML/app:Body/app:getExchangeRateForRateCodeResponse/app:ExchangeRateForRateCodeOutputVO/app:varCrncyCode}">
            <xsl:value-of select="/app:FIXML/app:Body/app:getExchangeRateForRateCodeResponse/app:ExchangeRateForRateCodeOutputVO/app:varCrncyUnits"/>
          </eo:VarCurrencyUnit>
        </eo:CurrencyExchange>
        <xsl:choose>
          <xsl:when test="count (/app:FIXML/app:Body/app:Error ) &lt;= 0">
            <com:Status>
              ...
              <com:Text>Exchange rate for rate code request processed successfully</com:Text>
            </com:Status>
          </xsl:when>
          <xsl:otherwise>
            <com:Status>
