      ...
      <eo:RsData>
        <eo:Cheque>
          <com:ChequeIdentifier>
            <com:ChequeID>
              <xsl:value-of select="/app:FIXML/app:Body/app:getChequesForChequebookResponse/app:ChequeInquiryResponseVO/app:chequeDetails/app:chequeNumber"/>
            </com:ChequeID>
          </com:ChequeIdentifier>
        </eo:Cheque>
        <com:Status>
          <xsl:choose>
            <xsl:when test="count (/app:FIXML/app:Body/app:Error) &lt;= 0">
              ...
              <com:Text>Cheque status inquiry processed successfully</com:Text>
            </xsl:when>
