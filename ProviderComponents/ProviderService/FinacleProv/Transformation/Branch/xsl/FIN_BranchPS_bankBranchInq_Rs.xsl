...
<xsl:stylesheet version="1.0" xmlns:oraxsl="http://www.oracle.com/XSL/Transform/java"
  ...
  <xsl:template match="/">
    <eo:ESBMsg>
      ...
      <eo:RsData>
        <xsl:for-each select="/app:FIXML/app:Body/app:executeFinacleScriptResponse/app:executeFinacleScript_CustomData/app:BankBranchInq_RES/app:BankBranchArray">
          <eo:Branch>
            <com:BranchIdentifier>
              <com:BankBIC>
                <xsl:value-of select="app:BankIdentifier"/>
              </com:BankBIC>
            </com:BranchIdentifier>
            <eo:BranchName>
              <xsl:value-of select="app:BranchName"/>
            </eo:BranchName>
            <eo:BankCode>
        ...
        <xsl:choose>
          <xsl:when test="count (/app:FIXML/app:Body/app:Error ) &lt;= 0">
            <com:Status>
              ...
              <com:Text>Bank branch inquiry processed successfully</com:Text>
            </com:Status>
          </xsl:when>
          <xsl:otherwise>
            <com:Status>
