...
<xsl:stylesheet version="1.0" xmlns:oraxsl="http://www.oracle.com/XSL/Transform/java"
  ...
  <xsl:template match="/">
    <eo:ESBMsg>
      ...
      <xsl:choose>
        <xsl:when test='/rs:ESBMsg/rs:RqData/rs:Account/rs:AccountType = "Mobile"'>
          <eo:RqData actionCode="QueryMobileAccountSummaryByPhone">
            <eo:Account>
              <custeo:Customer>
                <com:Phone>
                  <com:CompleteNumber>
                    <xsl:value-of select="concat('254',substring(/rs:ESBMsg/rs:RqData/rs:Account/rs:Customer/com:Phone/com:CompleteNumber,2))"/>
                  </com:CompleteNumber>
                </com:Phone>
              </custeo:Customer>
            </eo:Account>
        ...
        <xsl:otherwise>
          <xsl:if test="/rs:ESBMsg/rs:RqData">
            <eo:RqData actionCode="QueryBalanceSummaryByID">
              <xsl:if test="/rs:ESBMsg/rs:RqData/rs:Account">
                <xsl:for-each select="/rs:ESBMsg/rs:RqData/rs:Account">
                  <eo:Account>
